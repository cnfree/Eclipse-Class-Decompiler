...
public class LineNumberFormatter
{
	 ...
	 * 
	 * @param file
	 *            the file whose line numbers should be fixed
	 * @param lineNumberPositions
	 *            a recipe for how to fix the line numbers in 'file'.
	 * @param options
	 *            controls how 'this' represents line numbers in the resulting
	 *            file
	 */
			...
			EnumSet<LineNumberOption> options )
	{
		...
		_options = ( options == null ? EnumSet.noneOf( LineNumberOption.class )
				: options );
	}
	...
	private List<String> breakLines(
			List<LineNumberPosition> o_LineBrokenPositions ) throws IOException
	{
		int numLinesRead = 0;
		int lineOffset = 0;
		...
		try (BufferedReader r = new BufferedReader( new FileReader( _file ) ))
		{
			for ( int posIndex = 0; posIndex < _positions.size( ); posIndex++ )
			{
				...
				o_LineBrokenPositions.add( new LineNumberPosition( pos.getOriginalLine( ),
						pos.getEmittedLine( ) + lineOffset,
						pos.getEmittedColumn( ) ) );

				// Copy the input file up to but not including the emitted line
				// # in "pos".
				...
				// Read the line that contains the next line number annotations,
				// but don't write it yet.
				String line = r.readLine( );
				numLinesRead++;

				// See if there are two original line annotations on the same
				// emitted line.
				LineNumberPosition nextPos;
				int prevPartLen = 0;
				char[] indent = {};
				int indentLength = line.indexOf( line.trim( ) ) + 4;
				do
				{
					nextPos = ( posIndex < _positions.size( ) - 1 ) ? _positions.get( posIndex + 1 )
							: null;
					if ( nextPos != null
							&& nextPos.getEmittedLine( ) == pos.getEmittedLine( )
							&& nextPos.getOriginalLine( ) > pos.getOriginalLine( ) )
					{
						// Two different source line numbers on the same emitted
						// line!
						posIndex++;
						lineOffset++;
						String firstPart = line.substring( 0,
								nextPos.getEmittedColumn( ) - prevPartLen - 1 );
						brokenLines.add( new String( indent ) + firstPart );

						prevPartLen += firstPart.length( );

						indent = new char[indentLength];
						Arrays.fill( indent, ' ' );
						...
						o_LineBrokenPositions.add( new LineNumberPosition( nextPos.getOriginalLine( ),
								nextPos.getEmittedLine( ) + lineOffset,
								nextPos.getEmittedColumn( ) ) );
					}
					else
					{
						nextPos = null;
	...
	private void emitFormatted( List<String> brokenLines,
			List<LineNumberPosition> lineBrokenPositions ) throws IOException
	{
				...
				new BufferedWriter( new FileWriter( tempFile ) ) ))
		{
			...
			for ( LineNumberPosition pos : lineBrokenPositions )
			{
				...
				{
					// We currently need to remove newlines to squeeze things
					// together.
					// prefer to remove empty lines,
					// 1. read all lines before nextActual and remove empty
					// lines as needed
					...
					// 2. print non empty lines while stripping further as
					// needed
				...
				else
				{
					while ( numLinesRead < nextActual )
					{
						...
						int lineNoToPrint = isLast ? nextTarget
								: LineNumberPrintWriter.NO_LINE_NUMBER;

						if ( requiredAdjustment > 0 && doStretching )
						{
							// We currently need to inject newlines to space
							// things out.
							do
							{
								w.println( "" );
