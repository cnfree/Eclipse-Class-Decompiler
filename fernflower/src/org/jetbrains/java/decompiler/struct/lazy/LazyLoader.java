...
public class LazyLoader
{
	...
	public ConstantPool loadPool( final String classname )
	{
		try
		{
			final DataInputFullStream in = this.getClassStream( classname );
			if ( in == null )
			{
				return null;
			}
			...
			{
				in.discard( 8 );
				return new ConstantPool( (DataInputStream) in );
			}
			finally
			{
				in.close( );
