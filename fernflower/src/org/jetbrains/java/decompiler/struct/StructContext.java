...
public class StructContext
{
	...
	private void addSpace( String path, File file, boolean isOwn, int level )
	{
		...
		else
		{
			...
			if ( filename.endsWith( ".class" ) )
			{
				try
				{
					DataInputFullStream in = this.loader.getClassStream( file.getAbsolutePath( ), null );
					try
					{
						StructClass cl = new StructClass( in, isOwn, this.loader );
						this.classes.put( cl.qualifiedName, cl );
						unit.addClass( cl, filename );
						this.loader.addClassLink( cl.qualifiedName,
								new LazyLoader.Link( 1, file.getAbsolutePath( ), null ) );
					}
					finally
					{
						in.close( );
	...
	{
		ZipFile archive = type == 1 ? new JarFile( file ) : new ZipFile( file );
		try
		{
			...
			while ( entries.hasMoreElements( ) )
			{
				...
				ContextUnit unit = (ContextUnit) this.units.get( path + "/" + file.getName( ) );
				if ( unit == null )
				{
				...
				if ( !entry.isDirectory( ) )
				{
					if ( name.endsWith( ".class" ) )
					{
						...
						this.loader.addClassLink( cl.qualifiedName,
								new LazyLoader.Link( 2, file.getAbsolutePath( ), name ) );
					}
					else
					{
